...
#Posteriors in light colors and priors in dark
#Performance weighting
dens(rnorm(1e6, mean = 0, sd = 0.1), col = 'darkred', xlim = c(-0.35, 0.35), ylim = c(0,7), xlab = 'Beta value for performance rating', ylab = 'Density')
par(new = TRUE)
dens(d_loose$b_Weighting_conditionPerformance, col = 'red', xlim = c(-0.35, 0.35), ylim = c(0,7), xlab = 'Beta value for performance rating', ylab = 'Density', add = T)
par(new = TRUE)
dens(d_simple_l$b_Weighting_conditionPerformance, col = 'magenta', xlim = c(-0.35, 0.35), ylim = c(0,7), xlab = 'Beta value for performance rating', ylab = 'Density', add = T)

#Group
dens(rnorm(1e6, mean = 0, sd = 0.1), col = 'darkred', xlim = c(-0.35, 0.35), ylim = c(0,7), xlab = 'Beta value for performance rating', ylab = 'Density')
par(new = TRUE)
dens(d_loose$b_Group_affiliationParticipate, col = 'red', xlim = c(-0.35, 0.35), ylim = c(0,7), xlab = 'Beta value for performance rating', ylab = 'Density', add = T)
par(new = TRUE)
dens(d_simple_l$b_Group_affiliationParticipate, col = 'magenta', xlim = c(-0.35, 0.35), ylim = c(0,7), xlab = 'Beta value for performance rating', ylab = 'Density', add = T)

#Interaction
dens(rnorm(1e6, mean = 0, sd = 0.1), col = 'darkred', xlim = c(-0.35, 0.35), ylim = c(0,7), xlab = 'Beta value for performance rating', ylab = 'Density')
par(new = TRUE)
dens(d_loose$`b_Weighting_conditionPerformance:Group_affiliationParticipate`, col = 'red', xlim = c(-0.35, 0.35), ylim = c(0,7), xlab = 'Beta value for performance rating', ylab = 'Density', add = T)
...
# 
# SimList = c()
# for (S in 1:sim.params$nsims) {
#   SimList = c(SimList, rep(S, sim.params$ntrials))
# }
# 
...
# ScoresOut = c()
# for (S in 1:sim.params$nsims) {
#   ScoresOut = c(ScoresOut,symconfbaseline$All_avgbalanceOut[,S])
# }
# 
# d_out = cbind(ScoresOut,
#               SimList,
#               rep(1:50,sim.params$nsims),
#               rep("No_Weighting", sim.params$nsims*sim.params$ntrials),
#               rep("Vote", sim.params$nsims*sim.params$ntrials),
#               rep("Symmetrical", sim.params$nsims*sim.params$ntrials),
#               rep("Out", sim.params$nsims*sim.params$ntrials))
# 
# #In group
# ScoresIn = c()
# 
# for (S in 1:sim.params$nsims) {
#   ScoresIn = c(ScoresIn,symconfbaseline$All_avgbalanceIn[,S])
# }
# 
# d_in = cbind(ScoresIn,
#              SimList,
...
#              rep("Vote", sim.params$nsims*sim.params$ntrials),
#              rep("Symmetrical", sim.params$nsims*sim.params$ntrials),
#              rep("In", sim.params$nsims*sim.params$ntrials))
# 
# d1 = rbind(d_out, d_in)
# 
# 
...
# 
# for (S in 1:sim.params$nsims) {
#   ScoresOut = c(ScoresOut,symconfper$All_avgbalanceOut[,S])
# }
# 
# d_out = cbind(ScoresOut,
#               SimList,
#               rep(1:50,sim.params$nsims),
#               rep("Performance", sim.params$nsims*sim.params$ntrials),
#               rep("Vote", sim.params$nsims*sim.params$ntrials),
#               rep("Symmetrical", sim.params$nsims*sim.params$ntrials),
#               rep("Out", sim.params$nsims*sim.params$ntrials))
# 
...
# 
# for (S in 1:sim.params$nsims) {
#   ScoresIn = c(ScoresIn,symconfper$All_avgbalanceIn[,S])
# }
# 
# d_in = cbind(ScoresIn,
#              SimList,
